                       ...
                       <tbody>
                         <?php foreach ($list as $res) { ?>
                           <tr>
                             ...
                             <td>
                               <button type="button" class="btn btn-sm btn-flat btn-success btn-view" name="admin/barangay/edit" value="<?= $res['id']; ?>"> <i class="fa fa-edit"></i> Edit</button>
                               <?php if ($res['request_status_id'] == 4) { ?>
                                 <?php $redirect = ($res['purpose_id'] == 5) ? 'residency.php' : 'clearance.php'; ?>
                                 <a class="btn btn-sm btn-success btn-flat btn-print" href='<?= BASE_URL . "print/" . $redirect . "?pair=" . base64_encode($res['requester_id']) . "&ct_id=" . $res['id'] . "&code=" . base64_encode(date("Ymd", time() + 86400)); ?>"' target="_blank"><i class="fa fa-print"></i> Print</a>
                               <?php } else { ?>
